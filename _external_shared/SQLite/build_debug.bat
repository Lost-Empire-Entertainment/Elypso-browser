...
:: Set root and build directories for Debug
set "SQLITE_ROOT=%~dp0"
set "BUILD_DIR=%SQLITE_ROOT%build-debug"
set "INSTALL_DIR=%SQLITE_ROOT%install-debug"
...
cl /nologo /W4 /Zi /MDd /DDEBUG lemon.c /Felemon.exe || (
...
:: Build SQLite with Debug Flags
echo [INFO] Building SQLite for Debug...
nmake /f Makefile.msc USE_AMALGAMATION=1 USE_DLL=1 USE_FULLWARN=1 DEBUG=1 || (
    echo [ERROR] SQLite debug build process failed.
    pause
    exit /b 1
)

:: Install SQLite debug binaries and headers
echo [INFO] Installing SQLite (Debug binaries and libraries)...
...
echo [SUCCESS] SQLite Debug build and installation completed.
pause
exit /b 0
