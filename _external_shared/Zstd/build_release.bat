...
:: and disables tests, programs, and contributed binaries.

:: Set the root folder as the location of this script
set "MESON_ROOT=%~dp0"
set "BUILD_DIR=%MESON_ROOT%build-release"
set "INSTALL_DIR=%MESON_ROOT%install-release"
...
if not exist "%BUILD_DIR%" (
    ...
    meson setup "%BUILD_DIR%" --prefix "%INSTALL_DIR%" "%SOURCE_DIR%" -Dbin_tests=false -Dbin_programs=false -Dbin_contrib=false || (
