...
:: This batch file builds HarfBuzz from source code using MSVC (cl.exe) for Release.

:: Set the root folder as the location of this script
set "HARFBUZZ_ROOT=%~dp0"
set "BUILD_DIR=%HARFBUZZ_ROOT%build-release"
set "INSTALL_DIR=%HARFBUZZ_ROOT%install-release"
...
:: Configure HarfBuzz with CMake
echo [INFO] Configuring HarfBuzz with CMake...
cmake -G "Ninja" ^
...
:: Success message
echo [SUCCESS] HarfBuzz built and installed successfully.
echo -------------------------------------------------
